            ...
            <div className="lg:w-1/2 w-full bg-gradient-to-r from-transparent to-gray-50">
                ...
                <div className="w-full h-full flex flex-col justify-start pt-4 gap-10 pl-32">
                    <div className="w-full flex-col flex justify-start items-left font-medium mx-auto"> 
                        <input value={name} onChange={(e)=>setName(e.target.value)} placeholder="Họ và tên" className="w-2/3 border-[1px] border-gray-300 p-2"/>
                    </div> 
                    <div className="w-full flex-col flex justify-start items-left font-medium mx-auto"> 
                        <input value={numberPhone} onChange={(e)=>setNumberPhone(e.target.value)} placeholder="Số điện thoại" className="w-2/3 border-[1px] border-gray-300 p-2"/>
                    </div>
                    <div className="w-full flex-col flex justify-start items-left font-medium mx-auto"> 
                        <input value={address} onChange={(e)=>setAddress(e.target.value)} placeholder="Địa chỉ" className="w-2/3 border-[1px] border-gray-300 p-2"/>
                    </div> 
            ...
            <div className=" lg:w-1/2 w-full  h-full items-center justify-center flex bg-gradient-to-r from-gray-50 to-gray-100">
                <div className="lg:w-4/5 w-full pb-4 lg:pb-0 h-4/5 drop-shadow-[0_35px_35px_rgba(0,0,0,0.25)] bg-white rounded-lg flex flex-col justify-between">
                    ...
                    {/* <hr className="text-xl w-4/5 mx-auto"/> */}
                        <div className="py-2 bg-gray-200 max-h-32 overflow-auto">
                        {myCart &&
                        myCart.map((data)=>(
                            <div key={data.id} className="flex flex-row justify-between px-4 items-center">
                                <h1 className="font-medium">{data.name}</h1>
                                    <div className="flex flex-row justify-between gap-4 w-24">
                                        <div>x{data.count}</div><div>{formatter.format(+data.price.replace(/,/g, '')*data.count)}</div>
                                    </div>
                                </div>
                        )
